...
// DirectX初期化処理　ここから
    ...
    matRot *= XMMatrixRotationX(XMConvertToRadians(0.0f)); //X軸に回転
    matWorld *= matRot; //ワールド行列に回転行列を反映
    matRot *= XMMatrixRotationY(XMConvertToRadians(0.0f)); //Y軸に回転
    matWorld *= matRot;
    matRot *= XMMatrixRotationZ(XMConvertToRadians(0.0f)); //Z軸に回転
    matWorld *= matRot;
    ...
    //スプライト共通テクスチャ読み込み
    SpriteCommonLoadTexture(spriteCommon, 0, L"Resources/haikei.png", dev.Get());
    ...
    sprite[0].position.x = 0;
    sprite[0].position.y = 0;

    sprite[1].position.x = window_width / 2;
    sprite[1].position.y = window_height / 2;

    sprite[0].size.x = 1280.0f;
    sprite[0].size.y = 720.0f;

    sprite[1].size.x = 100.0f;
